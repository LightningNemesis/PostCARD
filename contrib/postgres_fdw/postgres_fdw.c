...
postgresImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
	...
	PG_TRY();
	{
		...
		appendStringInfoString(&buf,
							   "WHERE c.relkind IN (" CppAsString2(RELKIND_RELATION) "," CppAsString2(RELKIND_VIEW) "," CppAsString2(RELKIND_FOREIGN_TABLE) "," CppAsString2(RELKIND_MATVIEW) "," CppAsString2(RELKIND_PARTITIONED_TABLE) ") "
																																																										  "  AND n.nspname = ");
		deparseStringLiteral(&buf, stmt->remote_schema);
		...
		for (i = 0; i < numrows;)
		{
			...
			do
			{
				...
				attdefault = PQgetisnull(res, i, 4) ? (char *)NULL : PQgetvalue(res, i, 4);
				attgenerated = PQgetisnull(res, i, 5) ? (char *)NULL : PQgetvalue(res, i, 5);
				collname = PQgetisnull(res, i, 6) ? (char *)NULL : PQgetvalue(res, i, 6);
				collnamespace = PQgetisnull(res, i, 7) ? (char *)NULL : PQgetvalue(res, i, 7);
				...
				JoinPathExtraData *extra)
{
	...
	default:
		...
		elog(ERROR, "M_fdw unsupported join type %d", jointype);
	}
