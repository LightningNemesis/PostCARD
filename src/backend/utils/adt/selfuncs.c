...
Datum eqjoinsel(PG_FUNCTION_ARGS)
{
	...
	elog(NOTICE, "M_eqjoinsel: processing join type = %d", (int)sjinfo->jointype);
	...
	case JOIN_LEFT:
	case JOIN_FULL:
	case JOIN_HLL: // Just add this to the existing group
		selec = selec_inner;
		break;
	case JOIN_SEMI:
	case JOIN_ANTI:
	...
	default:
		...
		elog(ERROR, "M_eqjoinsel unrecognized join type: %d",
			 (int)sjinfo->jointype);
