...
pg_get_triggerdef_worker(Oid trigid, bool pretty)
{
	...
	appendStringInfo(&buf, " ON %s ",
					 pretty ? generate_relation_name(trigrec->tgrelid, NIL) : generate_qualified_relation_name(trigrec->tgrelid));
	...
	if (!attrsOnly)
	{
		if (!isConstraint)
			appendStringInfo(&buf, "CREATE %sINDEX %s ON %s%s USING %s (",
							 ...
							 idxrelrec->relkind == RELKIND_PARTITIONED_INDEX && !inherits ? "ONLY " : "",
							 (prettyFlags & PRETTYFLAG_SCHEMA) ? generate_relation_name(indrelid, NIL) : generate_qualified_relation_name(indrelid),
			 ...
			 int prettyFlags)
{
	...
	appendStringInfo(buf, " TO %s",
					 (prettyFlags & PRETTYFLAG_SCHEMA) ? generate_relation_name(ev_class, NIL) : generate_qualified_relation_name(ev_class));
						 ...
						 deparse_context *context)
{
	...
	else
	{
		 ...
		 */
		bool need_paren = (PRETTY_PAREN(context) || IsA(expr, FuncExpr) || IsA(expr, Aggref) || IsA(expr, WindowFunc) || IsA(expr, JsonConstructorExpr));
...
get_json_format(JsonFormat *format, StringInfo buf)
{
	...
	appendStringInfoString(buf,
						   format->format_type == JS_FORMAT_JSONB ? " FORMAT JSONB" : " FORMAT JSON");
	...
	if (format->encoding != JS_ENC_DEFAULT)
	{
		...
		encoding =
			format->encoding == JS_ENC_UTF16 ? "UTF16" : format->encoding == JS_ENC_UTF32 ? "UTF32"
																						  : "UTF8";
...
get_from_clause_item(Node *jtnode, Query *query, deparse_context *context)
{
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		case JOIN_RIGHT:
			...
			break;
		default:
			elog(ERROR, "M_get_from_clause_item unrecognized join type: %d",
				 (int)j->jointype);
		}
