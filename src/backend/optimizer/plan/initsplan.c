 ...
 */
void add_vars_to_targetlist(PlannerInfo *root, List *vars,
							Relids where_needed)
{
	ListCell *temp;
					...
					List **item_list)
{
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		elog(NOTICE, "deconstruct_recurse: handling join type = %d", (int)j->jointype);
		...
		default:
			...
			elog(ERROR, "M_initsplan.c unrecognized join type: %d",
				 (int)j->jointype);
