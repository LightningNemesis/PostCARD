								  ...
								  Relids *relids)
{
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		case JOIN_RIGHT:
			...
			break;
		default:
			elog(ERROR, "M_prepjointree_1.1 unrecognized join type: %d",
				 (int)j->jointype);
			break;
		}
		...
		*relids = bms_join(leftrelids, rightrelids);
		...
		elog(ERROR, "M_prepjointree_1.2 unrecognized node type: %d",
			 (int)nodeTag(jtnode));
	return jtnode;
}
						   ...
						   AppendRelInfo *containing_appendrel)
{
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		Assert(containing_appendrel == NULL);
		elog(NOTICE, "pull_up_subqueries_recurse: processing join type = %d", (int)j->jointype);
		...
		case JOIN_INNER:
			...
			j->rarg = pull_up_subqueries_recurse(root, j->rarg,
												 ...
												 NULL);
			break;
		case JOIN_HLL: // Add this case - handle like INNER JOIN
			j->larg = pull_up_subqueries_recurse(root, j->larg,
												 lowest_outer_join,
												 NULL);
			j->rarg = pull_up_subqueries_recurse(root, j->rarg,
		...
		case JOIN_RIGHT:
			...
			break;
		default:
			elog(ERROR, "M_prepjointree_2.1 unrecognized join type: %d",
				 (int)j->jointype);
			break;
		}
	}
	else
		elog(ERROR, "M_prepjointree_2.2 unrecognized node type: %d",
			 (int)nodeTag(jtnode));
	return jtnode;
}
						 ...
						 List *forced_null_vars)
{
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		case JOIN_ANTI:
			...
			break;
		default:
			elog(ERROR, "M_prepjointree_3 unrecognized join type: %d",
				 (int)jointype);
			break;
		}
							   ...
							   Relids *dropped_outer_joins)
{
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		j->larg = remove_useless_results_recurse(root, j->larg,
												 (j->jointype == JOIN_INNER) ? &j->quals : (j->jointype == JOIN_LEFT) ? parent_quals
																													  : NULL,
												 dropped_outer_joins);
		...
		elog(ERROR, "M_prepjointree_4 unrecognized node type: %d",
			 (int)nodeTag(jtnode));
	return jtnode;
}
