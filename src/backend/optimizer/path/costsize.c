						   ...
						   List *restrictlist)
{
	 ...
	 */
	elog(NOTICE, "calc_joinrel_size_estimate: processing join type = %d", (int)jointype);

	switch (jointype)
	{
	case JOIN_INNER:
	case JOIN_HLL: // Add this - handle like INNER JOIN
	...
	default:
		...
		elog(ERROR, "M_costsize unrecognized join type: %d", (int)jointype);
