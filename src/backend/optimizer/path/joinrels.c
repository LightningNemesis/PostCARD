							...
							SpecialJoinInfo *sjinfo, List *restrictlist)
{
	...
	elog(NOTICE, "M_joinrels: processing join type = %d", (int)sjinfo->jointype);
	...
	default:
		...
		elog(ERROR, "M_joinrels_1.1 unrecognized join type: %d", (int)sjinfo->jointype);
		break;
	}
					   ...
					   List *parent_restrictlist)
{
	...
	for (cnt_parts = 0; cnt_parts < joinrel->nparts; cnt_parts++)
	{
		...
		default:
			/* other values not expected here */
			elog(ERROR, "M_joinrels_1.2 unrecognized join type: %d",
