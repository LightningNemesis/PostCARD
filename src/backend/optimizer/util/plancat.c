					   ...
					   RelOptInfo *rel)
{
	 ...
	 */
	if ((inhparent && relation->rd_rel->relkind != RELKIND_PARTITIONED_TABLE) || (IgnoreSystemIndexes && IsSystemRelation(relation)))
		hasindex = false;
	else
 ...
 */
void estimate_rel_size(Relation rel, int32 *attr_widths,
				 ...
				 SpecialJoinInfo *sjinfo)
{
	...
	float8 result;
	JoinType modified_jointype = jointype; // Create a modified copy
	...
	elog(NOTICE, "join_selectivity: modified join type = %d", modified_jointype);
	...
	result = DatumGetFloat8(OidFunctionCall5Coll(oprjoin,
												 ...
												 ObjectIdGetDatum(operatorid),
												 PointerGetDatum(args),
												 Int16GetDatum(modified_jointype),
 ...
 */
void add_function_cost(PlannerInfo *root, Oid funcid, Node *node,
					   QualCost *cost)
{
	HeapTuple proctup;
