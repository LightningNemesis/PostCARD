 ...
 */
bool partition_bounds_equal(int partnatts, int16 *parttyplen, bool *parttypbyval,
				  ...
				  List **outer_parts, List **inner_parts)
{
	...
	while (outer_pos < outer_bi->ndatums || inner_pos < inner_bi->ndatums)
	{
		...
		/* Get the list values. */
		outer_datums = outer_pos < outer_bi->ndatums ? outer_bi->datums[outer_pos] : NULL;
		inner_datums = inner_pos < inner_bi->ndatums ? inner_bi->datums[inner_pos] : NULL;
							 ...
							 List **outer_parts, List **inner_parts)
{
	...
	for (i = 0; i < nmerged; i++)
	{
		...
		*outer_parts = lappend(*outer_parts, outer_index >= 0 ? outer_rel->part_rels[outer_index] : NULL);
		*inner_parts = lappend(*inner_parts, inner_index >= 0 ? inner_rel->part_rels[inner_index] : NULL);
	}
						...
						PartitionRangeBound *merged_ub)
{
	...
	default:
		elog(ERROR, "M_PartitionRangeBounds unrecognized join type: %d", (int)jointype);
	}
}
	...
	case PARTITION_STRATEGY_RANGE:
	{
		...
		if (partdesc->nparts > 0)
		{
			...
			else
			{
				 ...
				 */
				datum = cmpval == 0 ? linitial(spec->lowerdatums) : list_nth(spec->lowerdatums, abs(cmpval) - 1);
				overlap = true;
				overlap_location = datum->location;
